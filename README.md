...
- **People** — 人物 analyses photos on device (Vision face detection plus a feature print per face, from cached previews or a RAW's embedded JPEG; nothing leaves the Mac) and groups similar faces. Groups are suggestions: untick faces that aren't the person, then name the rest; naming another group the same merges them, 不是此人 takes a face back, and faces new analyses find close to a named person are offered for confirmation. Confirmed people tag their photos with `人物/名字` keywords, so search, smart albums and XMP carry them. The grouping reads appearance, not identity, so look-alike people can share a group — review before naming.
