...
struct MainToolbar: ToolbarContent {
    ...
    var body: some ToolbarContent {
        ...
        ToolbarItem {
            Toggle(isOn: filterOpen) {
                Label(app.filters.activeCount > 0 ? "筛选（\(app.filters.activeCount) 项）" : "筛选",
                      systemImage: app.filters.activeCount > 0
    ...
    private var canFilter: Bool { !app.isDuplicates && app.view != .analysis }

    private var viewMode: Binding<ViewMode> {
        Binding(get: { app.view }, set: { app.switchView($0) })
    }

    private var filterOpen: Binding<Bool> {
        Binding(get: { app.filterOpen }, set: { if $0 != app.filterOpen { app.toggleFilterBar() } })
    }
